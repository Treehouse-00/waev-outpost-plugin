...
## [0.9.390] - 2026-09-11
### Added
- **Messages on a phone: one list at a time.** The roster page used to stand channels and contacts side by side, each half a screen wide, which left neither wide enough to read a name. Now they stack: channels open to the full height and contacts folds to a single row under them that names it, shows how many of its conversations have something unread, and opens with a tap; opening contacts folds channels to its own row at the top the same way. Channels is open every time you arrive. The swap moves like a machine: the two screens trade heights in one short travel of eight detents that come closer as it ends, so the door slides with a mechanism's cadence and stops dead, and with your system set to reduce motion it cuts straight to the new state. On a desktop the three columns and the contacts fold to its icon column are unchanged.
- **Unread counts you can see.** In the Messages rosters the unread count is a lit cell now, the number in the panel's black on the display's own colour, at the right of the name, and a channel or contact with anything unread prints its name in the heavier weight. It was a faint grey chip that read as furniture. The count is part of the row itself, so tapping it opens the conversation rather than the remove key that used to share its spot, and every row reserves one piece at the end of its name line for it, flush with the head's keys, holding the count and, on a touch screen, the chevron under it, so the counts line up on one edge and nothing runs under them (the remove and options keys no longer take a track of their own; on a pointer screen they appear over that piece under the hand): a long channel name is cut with an ellipsis instead of spilling over, the preview line under the name runs the row's full width, and the Public channel's radio settings (frequency, spreading factor, bandwidth) move to a line of their own under the name, so Public runs three lines where the others run two. That line always stays one line: as the roster narrows the three chips take a smaller cut, and narrower still only the frequency prints. On a touch screen every roster row also ends in a small chevron, the way a phone's settings list says a row opens something, and a row with something unread draws its chevron heavier and brighter to say the same thing as its count: tap here.

### Changed
- **The companion keys carry the radio's full name.** The keys that pick which companion radio the console is holding were one narrow cap each and cut a name to a letter or three; they are three units wide now, about eighteen letters, and the row of them breaks onto a second row instead of running off the plate when there are more than two. On a phone the keys shrink a little together so the row still fits.
- **The bank's tallies stand as the screens do.** Channels before contacts, matching the channels screen at the left and the contacts screen at the right below them.
- **A shorter bank on a phone.** The monitor drops its three lamps there and prints two lines, the packet type and its channel on the first and the message under them, and shares the last row with the RF bar, monitor at the left and bar at the right, so the bank stays three rows tall with the wider keys.

---

## [0.9.389] - 2026-09-07
### Added
- **The console can hand the repeater's front door to the plugin.** Configuration → Web Frontend now lists every interface the repeater can serve at its root: the repeater's own, the hand-installed copy under /opt, and the waev:outpost plugin, each with its version. Pick one and the repeater switches at once, no restart. On a repeater that cannot list them yet, the two older choices remain.
- **One question after installing the plugin.** When the waev:outpost plugin finishes installing from the Plugins page, a small plate asks whether to make it your default experience. Yes makes the repeater serve it at the root and takes you there; No leaves things as they are. The choice stays under Configuration → Web Frontend.
### Changed
- **The hand-installed copy is marked as retiring.** Its row in Web Frontend carries an amber DEPRECATED tag and says plainly that the files under /opt and the pymc_console-dist repository will stop receiving releases, and that the plugin is the way forward.
- **The README leads with the plugin.** Install from the repeater's Plugins page in three presses, and let it update itself; the hand install is documented as the fallback.

## [0.9.388] - 2026-09-07
### Fixed
- **The console notices its own update.** The repeater's plugin manager keeps a ten-minute copy of the catalogue, and the console's timed checks were reading that copy, so a new release only showed after pressing refresh on the Plugins page. Every check now asks for a fresh catalogue, so the yellow pill and What's new light up on their own within a few seconds of the console settling and every half hour after.
- **The page reloads when the install finishes.** The console used to wait for its own request to answer before reloading, and on a real repeater that answer could arrive late or not at all. It now reloads the moment the repeater's progress stream reports the install complete, whatever the request is doing.

## [0.9.387] - 2026-09-07
### Added
- **A CHECK key in What's new.** When no newer console is known, the reader shows a small plate with a lamp, a readout saying what the last check found (NOT CHECKED, CHECKING, UP TO DATE, CHECK FAILED, or NO CATALOGUE on a repeater that cannot be asked), the running version, and one CHECK key that asks right away.
### Changed
- **Link health opens in a few seconds instead of fifteen.** The desk fetches its link histories four at a time rather than one after another with a pause between each. A week of 117 links that took about 15 seconds now lands in under 3.
- **The week view asks the repeater to summarise.** From a day up, the desk asks for ten-minute summaries of each link; a repeater that does not yet offer them (openHop Repeater before the summaries land) answers as before and the desk carries on with rows.
- **The update check runs once the console has settled.** Instead of a fixed minute after start, the first check runs a few seconds after the first stats are in and no history is loading, with the minute kept only as a ceiling.
### Fixed
- **Packet decoders on history work again without a repeater change.** Advert senders, names and locations, channel text, and control packets are now read from the payload the repeater already sends with every packet list, so the decoders fill in for history even though the repeater stopped sending whole frames in April. The console also no longer asks the repeater for frames it does not have, which removes one failed request per session.

## [0.9.386] - 2026-09-07
### Added
- **Every console learns about its own update.** The plugin build is recognised by how it was built, so a repeater that serves it as its main page at the root now checks the plugin catalogue and installs from What's new like any other plugin install. The standalone install, put in place with manage.sh, now reads the console's public release feed from your browser once a minute after start and every half hour; when a newer release is out, the version pill turns yellow and What's new names the release with a NOTES key and says that manage.sh upgrade on the repeater is the way to get it. A browser without internet sees nothing.
### Changed
- **The plugin card's keys are in two gangs.** What moves a plugin forward, install, update, open, restart, stands at the left; disable or enable and remove share one well at the right under MANAGE. The update key is blue with a download glyph that breathes in the cap's own light until it is pressed, and the console's own card no longer offers Open, since that page is the one you are reading. Wells are exactly as wide as their keys.
- **A blinking keycap legend breathes** over a slow 2.8-second cycle instead of flashing every half second.

## [0.9.385] - 2026-09-07
### Changed
- **The screens ship in phosphor green.** The display ink's default moves from the warm near-white to a green at hue 141, half saturation, brightness 94. If you never touched the three colour knobs on the Messages page, your screens follow the new default; if you only dimmed them, the dimming is kept and the ink changes; if you turned the hue or saturation by even one step, nothing changes for you. The reset key returns to the green.
- **A newer console is announced in yellow.** The version pill in the header turns the indicator yellow with a pulsing lamp when a newer waev:outpost is in the repeater's catalogue. Green read as a go; a pending update is a warning.
### Fixed
- **The console's own update check waits a minute after startup** and is skipped while the tab is hidden, so on a repeater without internet the thirty-second catalogue timeout never sits inside the first burst of requests that loads a page.

## [0.9.384] - 2026-09-06
### Changed
- **The update module is a proper instrument.** The notice in What's new is now a fastened plate: a lit band with a NEW lamp, an annunciator that reads UPDATE READY, INSTALLING, RELOADING, FAILED or STILL WORKING, and a version window; a screen that names the release in the reading face; and two keys in one well, NOTES to open the release and UPDATE to install it. While it installs, the screen lists the three steps with a clock beside the one in progress, and when the repeater streams its installer's output (a change proposed to openHop Repeater, pull request 445) the last five lines print under the steps as they arrive.
- **A slow install is no longer called a failure.** The console now waits as long as the repeater does for an install, fifteen minutes rather than three. If it still runs out of patience the module says STILL WORKING with an amber lamp and offers to reload the page, and if the repeater's stream is open and reports the install finished, the page reloads on its own.
### Fixed
- **Bold lead-ins and code in the release notes render as such**, and pip's indentation is kept in the installer's lines.

## [0.9.383] - 2026-09-06
### Added
- **The console tells you when a newer console is ready, and installs it.** Running as the openHop plugin, the console checks the repeater's plugin catalogue for a newer waev:outpost at start and every half hour. When one is there, the version pill in the header turns green with a pulsing lamp, and What's new opens on an update module: the release and the version change on a lit band, an explanation on the screen, and two keys, NOTES to open the release and UPDATE to install it on the repeater and reload this page into the new version. While it installs, the screen lists the three steps with a clock beside the one in progress; a failure shows the repeater's own words and the key reads RETRY. The console installed from an archive is not a plugin, so it keeps its usual path and shows none of this.
### Fixed
- **Release notes print their bold lead-ins as bold.** What's new used to show the asterisks around them.

## [0.9.382] - 2026-09-06
### Fixed
- **The packet detail's byte breakdown is back.** Newer repeater builds no longer include a packet's raw bytes in the packet lists, so a packet that arrived while the console was on another page opened to an empty left column. The console now fetches the bytes for any packet it opens without them, once per packet, and says so plainly if the repeater no longer has them either.
- **Packet history keeps its decoders.** The console asks the repeater's packet lists to include each packet's raw bytes, which restores sender resolution for adverts, trace signal paths and channel decryption on history loaded after a page reload. A repeater without that option answers once and is not asked again, so older builds lose nothing. This pairs with a change proposed to openHop Repeater (pull request 444); until it is installed the console behaves as before.

## [0.9.381] - 2026-09-05
### Added
- **A plugin browser and installer.** Configuration has a Plugins tab. It shows the openHop plugin catalogue and whatever is installed on the repeater as one rack of modules, each in the console's instrument grammar: a fastened plate carrying a screen with the plugin's picture and its category burned into the corner, a display printing its name, id and description, three pilot lamps (power, service, page) with a state annunciator and a version window, and the actions its state allows as keycaps under a printed bracket. The console you are reading marks its own module with a red tape. Install or update from the catalogue, enable or disable, restart a stopped service, open an application plugin's own page, or remove one after a confirm. A key in the header installs a wheel file from your computer. A filter narrows the roll, and a tally counts plugins, installs and updates.
- **Pictures that fit whatever a plugin brings.** Every card has one 16:9 frame. A share card fills it; a square icon stands centred on a field made of its own colours; a plugin with no picture prints its initials. The frame decides from the picture's own proportions when it loads, so a new plugin looks right without anyone declaring how.
- **Says what the repeater can do.** On a repeater from before the plugin manager the tab says so. On a repeater whose plugin manager service is not running it says that instead, with what to do about it.

- **The sidebar's controls as hardware.** The block at the foot of the sidebar is one fastened plate. Its top row is the front of a drawer: an illuminated pushbutton switch in the style of a tape transport's, CONTROLS printed on its lens. Pressing the switch lights it and slides a tray up from behind the plate carrying the advert keys (FLOOD and DIRECT, their pilot lenses blinking while an advert is on the air), the repeater mode as three latching keys (FWD, MON, NO TX) instead of a key that cycles, and the airtime limit as a key beside its percentage, all on one three-column grid. Pushing the front lowers the tray back in. Sessions and the noise floor read on seven-segment dies, the noise floor's samples draw as a scatter of readings on an LED screen the way the statistics page draws them, and the repeater's version prints in a pixel window that copies on a press or, when an update is ready, opens the updater. The noise floor now reads on every page rather than only the dashboard, map and statistics.

### Compatibility
- Needs an openHop Repeater from September 2026 or newer for the plugin manager; older repeaters get the notice above and nothing else changes. No repeater database change and no schema migration.

## [0.9.380] - 2026-09-05
### Added
- **The link scope shows one link at a time when you ask.** Choose a link and the scope draws that link alone, its ray and its peer, with everything else hidden. Press the same marker again, press empty glass, press anywhere on the plate that is not a control, or press the CLEAR key that appears at the right end of the scope's head band, and every link comes back.
- **Keys press down.** Every key in the console, keycaps, toggle keys and the round push buttons, drops one pixel in its frame as its face darkens under the finger and comes back up on release.

### Changed
- **The scope is centred on its glass.** Its head band no longer takes a row of the screen, so the drawing sits at the glass's middle instead of hanging low.
- **The glass falls off on every edge.** The tube's darkening at the left and right edges of reflective screens is half again as strong, and the top and bottom now fall off the same way, so the map, the terminal, the scope and the history strip read as a tube turning away on all four sides.

### Fixed
- **A hard line inside every screen edge is gone.** The room reflection every screen carries had a transparent margin and then an abrupt step a few pixels in from each edge, which stretched onto the glass as a visible line. The reflection now stretches from inside that border, so it meets the edge smoothly and keeps its soft dark rim.

### Under the hood
- Catalogue proposals for the openHop plugin catalogue are published through a scoped, short-lived GitHub App token instead of a personal token, attributed to the publishing bot, and opened ready for the catalogue's own certification to decide merging (contributed by yellowcooln).
...
## [0.9.379] - 2026-09-05
### Added
- **CAD Calibration has a front door.** It is a tab in the Configuration workspace, a key beside the LoRa parameters on the Radio Hardware page, and it carries a RADIO key back. It was reachable only by typing its address.
- **From a node to its link.** A node heard first-hand shows a Link history link in its map popup that opens Link Health on that node's link.
- **Statistics opens where you left it.** The sidebar's Statistics entry opens the statistics screen you last had open, and Link Health the first time; the workspace's tabs now read Link Health, Statistics, RF Health.
- **Neighbour Links is an instrument now: the Link Desk.** One plate built like the Messages companion's. Across the top, the tallies and the two controls that govern everything, the time window and what counts as active, as readouts and keys seated in the plate. Below them three columns: a roster that turns pages beside a bridge of LED meters, one meter lit to each listed link's RX score, with a draggable seam beside it; a framed module with its own screws that stacks whose link it is above a square scope on the map's reflective glass, which draws every link as a ray to its peer's bearing and distance, bright by its RX score; and a column of seven-segment readouts with a three-row screen of the links the scope cannot place under it, paging with its own keys, the plate's model print, a fine wireframe globe reading LINKMATE 2700, and the replay transport at the foot, clock, scrub fader, then the keys. Across the whole width under the three columns runs the RX history as a strip chart. Every screen's own controls (the roster's order and pages, the scope's range rings, ghosts and glare, the strip's metric) are a gang of keys in a well beside that screen, the gangs at the columns' feet all on one line, and nothing on the desk truncates its label. With no link selected the module's top line gives the neighbourhood's reach, its nearest and farthest placed peers, and the readouts read the totals. The roster's hover and open-row states are the Messages screen's, shared by every list printed on a screen. Nothing you came for is below the fold.
- **The desk replays the day.** Play, scrub or step through the window at ×10, ×60 or ×300, or press and drag on the history chart itself, on touch as well, and watch the links come and go: a ray fades with the silence since a peer was last heard, goes hollow and dashed when the peer is overdue against its own rhythm, and becomes a dim ghost past the active window. The history strip carries a playhead and dims what has not happened yet.
- **Shared hashes and unplaced peers are shown, not guessed.** A one-byte hash several neighbours share is drawn as an arc across their bearings with a count. The selected link's screen says so in plain words, lists the repeaters that share it with their distance and direction, and lets you pin the link to one of them. Links with no positioned neighbour are listed on the same screen as the shared hashes, under the readouts, still selectable. A repeater without a position gets the links spaced on a ring instead.
- **Every breakpoint has its own layout.** Three columns over the history strip on a wide screen, the roster beside the scope with the strip and the readouts below on a laptop, one column on a tablet and a phone, where choosing a link brings the scope into view.
- **Repeaters only.** A link is placed on the scope only against neighbours the repeater knows to be repeaters; a hash carried by companions alone is left out and counted, so the desk shows the repeater's links to other repeaters.
- **Point at anything on the scope.** A range-finder's brackets frame the marker under the pointer and a readout stands beside it: the peer, its range and bearing, its RX score, RSSI and SNR at the moment shown, when it was last heard and how many samples the repeater holds. Point at a ray and it lights up with a line explaining what it encodes, its length the range, its brightness the RX score dimmed by the silence since the peer was heard, dashed when overdue. Point at an arc and it explains the shared hash, names the repeaters it spans and their bearings, and says how to pin one. Point at the history strip and a cursor line follows the pointer while a readout in the strip's top-right corner gives the nearest observation's time and values; at rest the same corner carries the legend, so the strip has no head line and its picture takes the whole screen. The console's loading line runs along the desk's top edge while link histories are still arriving.

### Changed
- **Coloured keys print in white.** Every red, blue and amber key in the console (the map's view keys, the Messages companion's bank, the confirm dialogs, the Link Desk's Play key and its 7D and 1H presets) carries white lettering instead of black. The faces are dark enough that black read at two or three to one; white reads at six to nine.

### Fixed
- **Seven-segment readouts keep their width.** In a narrow rack a readout's digits were squeezed and clipped; the window now stays as wide as its digits need and the label beside it wraps instead.
