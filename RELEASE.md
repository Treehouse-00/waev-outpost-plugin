...
- After a successful public sync, `propose-plugin-catalogue.yml` downloads the
  ...
  bytes, and opens a version-specific ready PR against
  `openhop-dev/openhop-plugin-catalogue`. The catalogue-owned certification policy
  and required checks control automatic merging; the producer does not enable
  auto-merge or declare certification. Retries update the existing PR's evidence
  without changing a human-selected draft/ready state.
- Configure the `openhop-catalogue-publisher` GitHub App (ID `4844131`): install it
  on `openhop-dev/openhop-plugin-catalogue` with Contents read/write, Pull requests
  read/write, and Metadata read-only. In the producer source repository
  (`Treehouse-00/pymc_console`), store `OPENHOP_CATALOGUE_APP_ID` (value `4844131`)
  and `OPENHOP_CATALOGUE_APP_PRIVATE_KEY` (the App PEM private key) as **both required repository secrets**,
  not repository variables. Never print the private key. Keep artifact publication's
  `PUBLIC_REPO_TOKEN` separate; it is not a catalogue credential.
- Public catalogue checkout uses the read-only `github.token` without persisted
  credentials. Only after release-byte verification and the catalogue validator
  pass does the SHA-pinned `actions/create-github-app-token` v2.2.2 mint a
  short-lived installation token, restricted to that catalogue and those permissions.
  The action revokes it at job completion. The publish step uses the installation
  token for bot-authored PRs and resolves `openhop-catalogue-publisher[bot]` via the
  public users API for its numeric ID and bot noreply commit address; no personal
  `/user` endpoint or PAT is used.
