# openHop Console Release Guide

Complete guide for developers to create and publish new releases of openHop Console.
...
- It runs once per public repository, in parallel (a job matrix): the console
  ...
  which is what the repeater's plugin update check reads. Both repositories
  receive the same one-commit snapshot; the plugin one also carries
  `openhop-plugin.json`. `PUBLIC_REPO_TOKEN` must be able to push to both.
...
- It force-replaces public `main`, publishes the new release assets, and then
  deletes every older branch, tag, and GitHub Release.
- A hard postcondition requires exactly one root commit, one `main` branch, one
  tag, one latest GitHub Release, and no tracked `frontend/dist/` tree.

Generated UI files belong in the sole current public release, not the public
repository's Git history. The sync builds a fresh one-commit repository, keeps
staged UI and archives outside that checkout, and scans both metadata and staged
UI for secrets before publishing. Documentation is internal by default: only
`docs/images` crosses over, by allowlist, and the sync aborts before pushing if
anything else under `docs/` reached the staged tree. End-user clone examples retain
`--depth 1 --single-branch --branch main --no-tags` as a defense-in-depth bound.

Do **not** enable GitHub's repository-level immutable-release setting on the
distribution repository. That feature prevents deleting a published release and
its tag, so it is incompatible with retaining only one release. Here,
"latest-only invariant" means the workflow destructively replaces all public
refs/releases and then verifies the exact postcondition. Direct administrator
writes can still violate policy until the next successful release run.
...
`.github/workflows/build-ui-artifact.yml`, and
...
# Extract to the Console install target
