...
An existing [openHop Repeater](https://github.com/openhop-dev/openhop_repeater) installation. The Console dashboard is served by openHop Repeater's CherryPy server; installing it on its own does nothing useful.
...
This downloads the latest Console release, extracts to `/opt/pymc_console/web/html`, and — on a fresh install, when `yq` is present — patches `web.web_path` in `/etc/openhop_repeater/config.yaml`. Without `yq` the dashboard is still installed and the installer prints the command to run by hand. See [README.md](README.md) for all commands.
