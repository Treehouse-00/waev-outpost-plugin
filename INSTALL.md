...
This guide covers **manual** installation of the standalone waev:outpost dashboard (without `manage.sh`). For the standard installer workflow, see [README.md](README.md).

> [!TIP]
> **You probably don't need this page.** If your openHop Repeater has a **Plugins** page, open it, find **waev:outpost**, press **Install** and then **Enable**, and you're done. It will keep itself up to date from then on: when a new version is out, the version number at the top of the screen turns yellow, and one press of **Update** installs it. The steps below are only for a repeater without a Plugins page, or for people who want to place the files themselves.
